...
//                .fetch();
        ...
        //not return unique result exception
...
//        List<Member> content = results.getResults();
    ...
    //회원정렬순서 1.회원 나이 내림차순desc 2.회원 이름 올림차순 asc 2에서 이름이없으면 마지막에출력
    @Test
    public void sort() {
    ...
    public void aggregation() {
        ...
        assertThat(tuple.get(member.age.min())).isEqualTo(25);
    }
...
//                .on(team.name.eq("teamA"))
    ...
    public void concat() {
        //{username}_{age}
        List<String> result = queryFactory
                .select(member.username.concat("의나이").concat(member.age.stringValue()))
                .from(member)
                .where(member.username.eq("member1"))
                .fetch();
    ...
    public void bulkAdd() {
        long count = queryFactory
                .update(member)
                .set(member.age, member.age.add(1)) //multiply(x)
                .execute();
